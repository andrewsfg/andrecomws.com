...
if(isset($_POST['bt_cadcategoria'])){
	...
	if(isset($_POST['nome']) and !empty($_POST['nome']) and strlen($_POST['nome']) < 30){
		...
		//validando se LINK foi setado, se nгo estб vazio e se й maior que 30 caracteres

		if(isset($_POST['link']) and !empty($_POST['link']) and strlen($_POST['link']) < 30){

		//obtendo valor do campo LINK

		$categoria_link = $_POST['link'];
			...
			if($r_categoria == 0){
				...
				$categoria_pai = $_POST['categoriapai'];

				

			//inserindo categoria

			$ins_categoria = "INSERT INTO categoria(id_categoria_pai,nome,link,ordem) VALUES

			($categoria_pai,'$categoria_nome','$categoria_link',$categoria_ordem)";
...
header("location:cadcategoria_form.php");
